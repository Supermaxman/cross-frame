...
def preprocess_tweet(text: str, args: TweetPreprocessConfig):
    ...
    if args.replace_usernames:
        text = replace_usernames(text, filler=args.username_filler).replace(
            "[USER]", "@user"
        )
    if args.replace_urls:
        text = replace_urls(text, filler=args.url_filler).replace("[URL]", "")
    ...
    if args.replace_multiple_usernames:
        text = replace_multi_occurrences(text, args.username_filler).replace(
            "[USER]", "@user"
        )
