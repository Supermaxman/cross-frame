def check_results(response_schema, test_shuffled, test_problems):
    ...
    # 1,000 at a minimum should be 50 frames
    # use 5% for leeway
    if len(frames) < test_size * 0.05:
        print(
...
def stats_to_text(stats):
    ...
    lines.append(
        f"Seen Problems: {problems}/{total_problems}({problems/total_problems*100:.0f}%), Frames: {frames}"
    )
    return "\n".join(lines)
